 ...
 * </ul>
 * The behavior is controlled by the {@link #complexStructure} flag.
 * </p>
 * <p>
 * Preconfigured instances:
 * <ul>
 *   <li>{@code BUKKIT_LOADER} uses Bukkit's world container with a subfolder named "libraries" and locks the structure setting.</li>
 ...
 * Example usage:
 * <pre><code>
 * // In your plugin's onLoad or onEnable:
 * DependencyLoader loader = DependencyLoader.BUKKIT_LOADER;
 * loader.load("com.example", "my-library", "${version}", DependencyLoader.MAVEN_REPO_URLS[0]);
 *
...
public class DependencyLoader {
     ...
     * <p>
     * Note: The folder structure for this loader is fixed; the {@link #complexStructure} flag for this instance
     * cannot be changed.
     * </p>
     */
     ...
     *
     * @param log     the log level enum value (GOOD, BAD, or ERROR).
    ...
    private boolean downloadFile(String urlString, File destiny) throws IOException {
        ...
        // First, perform a HEAD request to check if the URL is reachable.
        try {
     ...
     * It downloads the JAR if it does not exist (using a complex or flat structure based on the
     * {@code complexStructure} flag) and loads it dynamically via reflection.
     * </p>
     *
     * @param group    the dependency group ID.
     * @param artifact the dependency artifact ID.
     * @param version  the dependency version (this can be set as a variable depending on the release tag).
    ...
    public final boolean load(String group, String artifact, String version, String repoUrl) {
        try {
            // Build the path string using file separators for cross-platform compatibility.
     ...
     * <p>
     * The YAML file must have a {@code .yml} extension and contain dependency definitions.
     * </p>
     *
    ...
    private boolean loadFromConfiguration(FileConfiguration c) {
        ...
        for (Map<?, ?> map : dependencies) {
            ...
            loadAtLeastOne = load(group, artifact, version, repo);
        }
     ...
     * @param folderName      the subfolder name to use (might be {@code null}).
